...
class ThumbnailImageFieldFile(ImageFieldFile):
    ...
    def save(self, name, content, save=True):
        ...
        # 썸네일 크기로 이미지를 저장할 새로운 파일 생성
        thumb_img = Image.new('RGB', size, (255, 255, 255))
        img_position = ((size[0] - img.size[0]) // 2, (size[1] - img.size[1]) // 2)
        thumb_img.paste(img, img_position)
        thumb_img.save(self.thumb_path, 'JPEG')
